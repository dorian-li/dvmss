...
for line, line_d in flt_d.groupby("line"):
    ...
    sim_d = sim.dpred()
    sim_d = pred_data_to_df(sim_d)
    sim_d.index = line_d.index
    print(line)
    print(sim_d)
    sim_ds = pd.concat([sim_ds, sim_d])

sim_ds = pd.concat([flt_d["line"], sim_ds], axis=1)
sim_ds.to_csv("sim_ds.csv", index=False)
